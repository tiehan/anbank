...
import logbook_zmqpush
# import multiprocessing
import os
import time
import sys

from anbank import filehandler
# from genongs.filehandler import file_checker
# from genongs.log import *
...
def _create_log_handler(config, add_hostname=False, direct_hostname=False):
    ...
    format_str = "".join(["[{record.time:%Y-%m-%dT%H:%M}] " if config.get("include_time", True) else "",
...
def setup_script_logging():
    ...
    """
    handlers = [logbook.NullHandler()]
    format_str = ("[{record.time:%Y-%m-%dT%H:%M}] "
